...
nu_Window nu_create_window(const char *name, size_t width, size_t height, bool fullscreen);
// Destroys a window
...
// the shader type from the file extension. Check C source file for all extensions
...
// component_types = the individual type of each attribute (vec3 = GL_FLOAT, float = GL_FLOAT, int = GL_INT)
